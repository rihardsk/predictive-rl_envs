...
import org.rlcommunity.btanner.agentLib.actionSelectors.ProvidesActionProbabilitiesFromValues;
import org.rlcommunity.btanner.agentLib.actionSelectors.ProvidesActionValues;
 ...
 * @author btanner
 */
public class ExpectedExperienceReplayLambdaLearningBooster implements LearningBoosterInterface {
    ...
    public static void addToParameterHolder(ParameterHolder p) {
        ...
        p.addDoubleParam("e-replay-lambda", 0.0d);
    }

    public ExpectedExperienceReplayLambdaLearningBooster(TaskSpec theTaskObject, ParameterHolder theParameterHolder, LearningModuleInterface theLearningModule, ActionSelectorInterface theActionSelector) {
        ...
        this.lambda = theParameterHolder.getDoubleParam("e-replay-lambda");
        theLessons = new Vector<Lesson>();

        if (!(theActionSelector instanceof ProvidesActionValues)) {
            throw new InvalidParameterException("" + getClass() + " needs an action selector that implements: ProvidesActionValues");
        }
        if (!(theActionSelector instanceof ProvidesActionProbabilitiesFromValues)) {
            throw new InvalidParameterException("" + getClass() + " needs an action selector that implements: ProvidesActionProbabilitiesFromValues");
        }
    ...
    private void learnFromLesson(Lesson theLesson) {
        ...
        double lastNextValue = 0.0d;
        int lastNextAction = 0;

        for (int t = theLesson.size() - 1; t >= 0; t--) {
            DataPoint thisStep = theLesson.getData(t);
            double nextTDLambdaValue = 0.0d;
            double nextTD0Value = 0.0d;

            //Last step of the lesson
            if (t == theLesson.size() - 1) {
                if (!thisStep.isAbsorbing()) {
                    double[] actionValues = ((ProvidesActionValues) theActionSelector).getActionValues(thisStep.getSPrime(), theLearningModule);
                    double[] actionProbs = ((ProvidesActionProbabilitiesFromValues) theActionSelector).getActionProbabilities(actionValues);
                    for (int i = 0; i < actionValues.length; i++) {
                        nextTD0Value += actionValues[i] * actionProbs[i];
                    }
                }
                nextTDLambdaValue = nextTD0Value;
            }
//Every other step of the lesson
            if (t != theLesson.size() - 1) {
                if (!thisStep.isAbsorbing()) {
                    double[] actionValues = ((ProvidesActionValues) theActionSelector).getActionValues(thisStep.getSPrime(), theLearningModule);
                    double[] actionProbs = ((ProvidesActionProbabilitiesFromValues) theActionSelector).getActionProbabilities(actionValues);
                    for (int i = 0; i < actionValues.length; i++) {
                        nextTD0Value += actionValues[i] * actionProbs[i];
                        if (lastNextAction != i) {
                            nextTDLambdaValue += actionValues[i] * actionProbs[i];
                        } else {
                            nextTDLambdaValue += lastNextValue * actionProbs[i];

                        }
                    }
                }
            }
            double blendedNextValue = lambda * nextTDLambdaValue + (1.0d - lambda) * nextTD0Value;
            double r = thisStep.getReward();
            double target = r + blendedNextValue;
            ...
            lastNextAction = thisStep.getAction();
            lastNextValue = target;
        }
    }
